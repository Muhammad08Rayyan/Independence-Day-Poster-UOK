...
export class StyleRenderer {
  ...
  private drawBackground(context: CanvasRenderContext): void {
    ...
    if (this.style.id === 'style1') {
      ...
      bgImg.onload = () => {
        ...
        // Place logo below the message box (left layout in style 1)
        const logoImg = new Image();
        logoImg.crossOrigin = 'anonymous';
        logoImg.onload = () => {
          // Compute approximate message box area for style 1
          const msgX = 20;
          const msgY = this.config.layout.messageY;
          const msgWidth = width * 0.55;
          const msgHeight = this.config.layout.messageHeight;

          // Position logo centered under the message box
          const baseLogoWidth = 140;
          const baseLogoHeight = 140;
          const logoWidth = Math.round(baseLogoWidth * 0.8);  // 80% of previous size
          const logoHeight = Math.round(baseLogoHeight * 0.8); // 80% of previous size
          const logoX = msgX + (msgWidth - logoWidth) / 2;
          const logoY = msgY + msgHeight + 16 - 10; // move 10px up
          ctx.drawImage(logoImg, logoX, logoY, logoWidth, logoHeight);

          // Now draw the rest of the content on top
          this.drawContentForStyle1(context);
        };
        logoImg.onerror = () => {
          // If logo fails to load, still draw content
          this.drawContentForStyle1(context);
        };
    ...
    if (this.style.id === 'style3') {
      ...
      bgImg.onload = () => {
        ...
        // Overlay: replace small flag at top-left with logo.png (if available)
        const logoImg = new Image();
        logoImg.crossOrigin = 'anonymous';
        logoImg.onload = () => {
          const padding = 30;
          const size = Math.round(150 * 0.9); // 90% of previous size
          // Cover the original flag area so it doesn't show beneath the logo
          ctx.save();
          ctx.fillStyle = 'rgba(250,253,248,255)';
          // Conservative cover area over top-left region where the small flag sits
          ctx.fillRect(0, 0, Math.max(240, padding + size + 20), Math.max(180, padding + size - 10));
          ctx.restore();
          ctx.drawImage(logoImg, padding, padding, size, size);
          // Draw all content after overlay
      ...
      bgImg.onerror = () => {
        ...
        // Try overlaying logo on fallback as well
        const logoImg = new Image();
        logoImg.crossOrigin = 'anonymous';
        logoImg.onload = () => {
          const padding = 30;
          const size = Math.round(150 * 0.9);
          ctx.save();
          ctx.fillStyle = '#FFFFFF';
          ctx.fillRect(0, 0, Math.max(240, padding + size + 20), Math.max(180, padding + size - 10));
          ctx.restore();
  ...
  public drawProfile(context: CanvasRenderContext, data: PosterRenderData): void {
    ...
    if (this.style.id === 'style1') {
      ...
      // Default positioning (no style2 anymore)
      centerX = width / 2;
      centerY = this.config.layout.profileY;
      radius = this.config.layout.profileRadius;
    }
    ...
    if (this.config.decorations.stars && this.style.id !== 'style3') {
  ...
  public drawFooter(context: CanvasRenderContext): void {
    ...
    if (this.style.id === 'style1') {
      ...
      footerBgColor = this.config.colors.primary; // Use config color
    }
    ...
    ctx.textAlign = 'center';
    ctx.fillText('UNIVERSITY OF KARACHI', width / 2, footerY + 30);
  }
...
export function renderStyle2WithBackground(
  ...
  const renderer = createStyleRenderer('style3');
  ...
  bgImg.onerror = function() {
    // Fallback to regular Style 3 rendering if background fails
    renderPosterWithStyle(canvas, 'style3', data);
  };
